...
from emp.orderly_types import Interval, PerpetualAssetType
...
class Position(BaseModel):
    ...
    def size(self) -> float:
        ...
        return sum(trade.size for trade in self.broker.trades)

    @property
    def pl(self) -> float:
        """Profit (positive) or loss (negative) of the current position in cash units."""
        return sum(trade.pl for trade in self.broker.trades)

    @property
    def pl_pct(self) -> float:
        """Profit (positive) or loss (negative) of the current position in percent."""
        weights = np.abs([trade.size for trade in self.broker.trades])
        weights = weights / weights.sum()
        pl_pcts = np.array([trade.pl_pct for trade in self.broker.trades])
    ...
    def __repr__(self):
        return f"<Position: {self.size} ({len(self.broker.trades)} trades)>"
...
class Order(BaseModel):
    ...
    tag: object | None = None  # Arbitrary value (such as a string) which, if set, enables tracking of this order and the associated `Trade` (see `Trade.tag`).
    ...
    def cancel(self):
        ...
        if trade:
            if self is trade.sl_order:
                trade._replace(sl_order=None)
            elif self is trade.tp_order:
...
class Trade(BaseModel):
    ...
    broker: "_Broker"
    size: int = Field(description="Trade size (volume; negative for short trades).")
    entry_price: float
    exit_price: Optional[float] = None
    exit_bar: Optional[int] = Field(default=None, description="Candlestick bar index of when the trade was exited (or None if the trade is still active).")
    sl_order: Optional[Order] = None
    tp_order: Optional[Order] = None
    entry_bar: int = Field(description="Candlestick bar index of when the trade was entered.")
    tag: object = Field(description="""
    ...
    def close(self, portion: float = 1.0):
        ...
        order = Order(broker=self.broker, size=size, parent_trade=self, tag=self.tag)
    ...
    def entry_time(self) -> Union[pd.Timestamp, int]:
        ...
        return self.broker._data.index[self.entry_bar]
    ...
    def exit_time(self) -> Optional[Union[pd.Timestamp, int]]:
        ...
        if self.exit_bar is None:
            return None
        return self.broker._data.index[self.exit_bar]
...
class _Broker(BaseModel):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(position=Position(broker=self))
    ...
    def new_order(
        ...
        *,
        ...
        order = Order(broker=self, size=size, limit_price=limit, stop_price=stop, sl_price=sl, tp_price=tp, parent_trade=trade, tag=tag)
    ...
    def _process_orders(self):
        ...
        for order in list(self.orders):  # type: Order
            ...
            if order.parent_trade:
                ...
                if order in (trade.sl_order, trade.tp_order):
    ...
    def _reduce_trade(self, trade: Trade, price: float, size: float, time_index: int):
        ...
        if not size_left:
            ...
            if trade.sl_order:
                trade.sl_order._replace(size=-trade.size)
            if trade.tp_order:
                trade.tp_order._replace(size=-trade.size)
    ...
    def _open_trade(
        ...
        trade = Trade(broker=self, size=size, entry_price=price, entry_bar=time_index, tag=tag)
...
class EmpOrderly(BaseModel):
    ...
    _mp_backtests: ClassVar[dict[float, tuple["EmpOrderly", list, Callable]]] = {}

    asset: PerpetualAssetType | None = Field(None)
    cash: float = 10_000
    commission: float = (
        0.0  # `commission` must be a float value, percent of " "entry order price
    )
    margin: float = 1.0
    sdk: EmpyrealOrderlySDK = Field(default_factory=lambda: EmpyrealOrderlySDK(
        account_id=os.environ['ACCOUNT_ID'],
        pvt_hex=os.environ['PVT_HEX'])
    )
    trade_on_close: bool = False
    hedging: bool = False
    exclusive_orders: bool = False
    _strategy: Type[Strategy] = PrivateAttr()
    ...
    def _init_data(
        ...
        if np.any(data["close"] > self.cash):
            warnings.warn(
    ...
    async def load_data(self, asset: PerpetualAssetType | None = None, lookback: int = 21, interval: Interval = Interval.hour):
        if (asset or self.asset):
            _asset: PerpetualAssetType = asset or self.asset  # type: ignore
        else:
            raise ValueError("must provide an asset")
        self._history = await self.sdk.get_ohlcv(resolution=interval, start_days_behind=lookback, asset=_asset)
        self._init_data(self._history)
        return self

    def backtest(self, asset: PerpetualAssetType | None = None, lookback: int = 21, interval: Interval = Interval.hour, **kwargs) -> pd.Series:
        """
        ...
        try:
            ...
            raise AttributeError("Load data using .load_data()")

        data = _Data(self._history.copy(deep=False))
    ...
    def optimize(
        ...
        * `"skopt"` which finds close-to-optimal strategy parameters using
        ...
        if isinstance(maximize, str):
            ...
            stats = self._results if self._results is not None else self.backtest()
        ...
        def _optimize_grid() -> tuple[pd.Series, pd.Series, pd.Series]:
            ...
            EmpOrderly._mp_backtests[backtest_uuid] = (self, param_batches, maximize)  # type: ignore
            try:
                ...
                if mp.get_start_method(allow_none=False) == "fork":
                    with ProcessPoolExecutor() as executor:
                        futures = [
                            executor.submit(EmpOrderly._mp_task, backtest_uuid, i)
                            for i in range(len(param_batches))
                        ]
                        for future in _tqdm(
                            as_completed(futures),
                            total=len(futures),
                            desc="EmpOrderly.optimize",
                        ):
                    ...
                    for batch_index in _tqdm(range(len(param_batches))):
                        _, values = EmpOrderly._mp_task(backtest_uuid, batch_index)
                        for value, params in zip(values, param_batches[batch_index]):
                            heatmap[tuple(params.values())] = value
            finally:
                del EmpOrderly._mp_backtests[backtest_uuid]
            ...
            if pd.isnull(best_params):
                ...
                stats = self.backtest(**param_combos[0])
            else:
                stats = self.backtest(**dict(zip(heatmap.index.names, best_params)))

            return stats, best_params, heatmap
        ...
        def _optimize_skopt() -> Union[
            ...
            memoized_run = lru_cache()(lambda tup: self.backtest(**dict(tup)))
            ...
            stats = self.backtest(**dict(zip(kwargs.keys(), res.x)))
            output = [stats]
    ...
    def _mp_task(backtest_uuid, batch_index):
        bt, param_batches, maximize_func = EmpOrderly._mp_backtests[backtest_uuid]
        return batch_index, [
            maximize_func(stats) if stats["# Trades"] else np.nan
            for stats in (bt.backtest(**params) for params in param_batches[batch_index])
        ]
    ...
    def plot(
        ...
        *,
        ...
        `EmpOrderly.backtest` or
        `EmpOrderly.optimize`, otherwise the last
        ...
        return plot(
            ...
            show_price_data=show_price_data,
        )
